...
-- only group fields and aggregate functions are allowed for SELECT queries with aggregation, so to include reg_id include it as a group field. The reg_code always matches the reg_name anyway.
...
SELECT dis_name, count(dis_name) as cntward
from tanzania_pop_ward
...
SELECT adminwards.*, total_both as totalpop, total_male as male, total_fema as female
FROM adminwards LEFT JOIN tanzania_pop_ward
ON adminwards.ward_name = tanzania_pop_ward.ward_name AND adminwards.district_n = tanzania_pop_ward.dis_name;
...
FROM
	...
	FROM wards LEFT JOIN tanzania_pop_ward
...
SELECT *, st_area(utmgeom)/1000000 as sqkm
FROM ward_census;
...
/*attribute join or query instead of GIS spatial tools*/
